...
func OSSkeyinit() {
	...
	bucket, err = client.Bucket("pathcystore")

	if err != nil {
		fmt.Println("Error:", err)
		os.Exit(-1)
	}
	fmt.Println("已连接bucket"+bucket.BucketName)
...
//	fp :=absPath+name
	...
	err := bucket.PutObjectFromFile("video/"+name, absPath+"\\video\\"+name)
	if err != nil {
		fmt.Println("Error:", err)
		os.Exit(-1)
	}
	err =bucket.SetObjectACL("video/"+name,oss.ACLPublicRead)
	if err != nil {
		fmt.Println("Error:", err)
		os.Exit(-1)
	}
...
func UploadCover(name string,wg *sync.WaitGroup) {
	...
	err := bucket.PutObjectFromFile("cover/"+name+".jpeg", absPath+"\\cover\\"+name+".jpeg")
	if err != nil {
		fmt.Println("Error:", err)
		os.Exit(-1)
	}
	err =bucket.SetObjectACL("cover/"+name+".jpeg",oss.ACLPublicRead)
	if err != nil {
		fmt.Println("Error:", err)
		os.Exit(-1)
	}
